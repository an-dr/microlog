...
/// @return ULOG_STATUS_OK on success, ULOG_STATUS_BAD_ARGUMENT if invalid parameters
...
/// @return ULOG_STATUS_OK on success, ULOG_STATUS_INVALID_ARG if invalid parameters
...
/// @return Pointer to struct tm with time information, or NULL if event is NULL or time feature disabled
...
/// @brief Enable or disable source location in logs (requires ULOG_FEATURE_SOURCE_LOCATION)
...
/// @param use_short_levels True to use short level strings (T/D/I/W/E/F), false for long (TRACE/DEBUG/...)
...
/// @brief Sets the custom prefix generation function (requires ULOG_FEATURE_PREFIX)
...
/// @return ULOG_STATUS_OK on success, ULOG_STATUS_BAD_ARGUMENT if invalid parameters, ULOG_STATUS_ERROR if output not found
...
/// @param ... Topic name string and format arguments
