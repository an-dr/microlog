...
void test_ulog_add_fp() {
    ...
    const char *message_to_log = "Hello to file from ulog_add_fp test with varargs!"; // Restored and updated
    log_info("Message for fp test: %s", message_to_log); // INFO is >= DEBUG
    ...
    assert(strstr(buffer, message_to_log) != NULL && "Message with varargs not found in temp file output");
