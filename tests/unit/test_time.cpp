...
static void _check_date_time_fields(int yr, int mo, int d, int hh, int mm, int ss, 
                                    const time_t &before, const time_t &after) {
    auto before_tm = gmtime(&before);
...
void _check_console_time() {
    ...
    int hh, mm, ss;
    REQUIRE(sscanf(ut_callback_get_last_message(), "%d:%d:%d ", &hh, &mm, &ss) == 3);
...
void _check_file_time() {
    ...
    REQUIRE(sscanf(buffer, "%d-%d-%d %d:%d:%d ", &yr, &mo, &d, &hh, &mm, &ss) == 6);
