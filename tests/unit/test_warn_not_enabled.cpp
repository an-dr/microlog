...
void warning_test_callback(ulog_event *ev, void *arg) {
    ...
    if (ev && ulog_event_get_level(ev) == ULOG_LEVEL_WARN) {
        ulog_event_to_cstr(ev, warning_last_message,
                           sizeof(warning_last_message));
    }
}
...
TEST_CASE_FIXTURE(WarnNotEnabledTestFixture, "Color Config Warning") {
    ...
    ulog_output_id warning_output =
        ulog_output_add(warning_test_callback, nullptr, ULOG_LEVEL_WARN);
    REQUIRE(warning_output != ULOG_OUTPUT_INVALID);

    reset_warning_callback();

    // Call color config function (should be disabled in base config and
    // generate warning)
...
TEST_CASE_FIXTURE(WarnNotEnabledTestFixture, "Prefix Config Warning") {
    ...
    CHECK(strstr(warning_last_message, "ULOG_BUILD_DYNAMIC_CONFIG disabled") !=
          nullptr);
...
TEST_CASE_FIXTURE(WarnNotEnabledTestFixture, "Source Location Config Warning") {
    ...
    // Call source location config function (should be disabled and generate
    // warning)
    ulog_status result = ulog_source_location_config(false);

    CHECK(result == ULOG_STATUS_DISABLED);
    CHECK(warning_callback_count == 1);
    CHECK(strstr(warning_last_message, "ulog_source_location_config") !=
          nullptr);
    CHECK(strstr(warning_last_message, "ULOG_BUILD_SOURCE_LOCATION disabled") !=
          nullptr);
...
TEST_CASE_FIXTURE(WarnNotEnabledTestFixture, "Topic Functions Warnings") {
    ...
    SUBCASE("Topic Level Set Warning") {
        ...
        ulog_status result =
            ulog_topic_level_set("test_topic", ULOG_LEVEL_INFO);

        CHECK(result == ULOG_STATUS_DISABLED);
        CHECK(warning_callback_count == 1);
        CHECK(strstr(warning_last_message, "ulog_topic_level_set") != nullptr);
        CHECK(strstr(warning_last_message, "ULOG_BUILD_TOPICS_MODE disabled") !=
              nullptr);
    }
    ...
    SUBCASE("Topic Add Warning") {
        ...
        ulog_topic_id result =
            ulog_topic_add("test_topic", ULOG_OUTPUT_ALL, ULOG_LEVEL_INFO);
...
TEST_CASE_FIXTURE(WarnNotEnabledTestFixture,
                  "Extra Output Functions Work In Base Config") {
    ...
    SUBCASE("Output Add Function Available") {
        ...
        ulog_output_id result =
            ulog_output_add(warning_test_callback, nullptr, ULOG_LEVEL_TRACE);
    ...
    SUBCASE("Output Add File Function Available") {
        ...
        ulog_output_id result =
            ulog_output_add_file(temp_file, ULOG_LEVEL_TRACE);
        CHECK(result != ULOG_OUTPUT_INVALID);
        ulog_cleanup();
        fclose(temp_file);
    }
...
TEST_CASE_FIXTURE(WarnNotEnabledTestFixture,
                  "Prefix Set Function Works In Base Config") {
    // In the base config, ULOG_BUILD_PREFIX_SIZE=16, so prefix functions should
    // work

    // Define a dummy prefix function
    static auto dummy_prefix_fn = [](ulog_event *ev, char *prefix,
                                     size_t prefix_size) {
        (void)ev;
        (void)prefix;
        (void)prefix_size;
    ...
    // Call prefix set function (should work since ULOG_BUILD_PREFIX_SIZE=16 in
    // base config)
...
TEST_CASE_FIXTURE(WarnNotEnabledTestFixture, "Warning Message Format") {
    ...
    // Verify warning message format: "'function_name' called with FEATURE
    // disabled"
    CHECK(warning_callback_count == 1);
    CHECK(strstr(warning_last_message,
                 "'ulog_color_config' called with ULOG_BUILD_COLOR disabled") !=
          nullptr);
...
TEST_CASE_FIXTURE(WarnNotEnabledTestFixture, "Warning Level Filtering") {
    ...
    ulog_output_id error_output =
        ulog_output_add(warning_test_callback, nullptr, ULOG_LEVEL_ERROR);
